...
async function createCampaignAction({ formData }: CreateCampaignActionArgs) {
	...
	await db.transaction(async (trx) => {
		...
		const newCampaignSchedule: InsertCampaignScheduleModel = {
			...
			dayOfWeekList: submission.value.dayOfWeek,
				...
				cell: ({ row }) => {
					...
					if (dayOfWeeks.includes(now.getDay())) {
						...
						if (isBefore(campaignScheduleStart, campaignScheduleEnd)) {
							...
							return format(campaignScheduleStart, 'dd/MM/yyyy')
						}
					}
...
/*
		...
		schema: addPropertySchema.superRefine(async (data, ctx) => {
			const {
				areaCode,
				neighborhoodNumber,
				subAreaNumber,
			...
			const existingProperty = await db.query.properties.findFirst({
				where: (properties, { eq, and }) =>
					and(
						eq(properties.propertyNumber, propertyNumber),
						eq(properties.areaName, areaName),
						eq(properties.areaCode, areaCode),
						eq(properties.neighborhoodName, neighborhoodName),
						eq(properties.neighborhoodNumber, neighborhoodNumber),
						eq(properties.subAreaName, subAreaName),
						eq(properties.subAreaNumber, subAreaNumber),
					),
				columns: {
					id: true,
				},
			...
			const errorMessage = `A property already exists with the following details:
			- Property Number: ${propertyNumber}
			- Area: ${areaName} (Code: ${areaCode})
			- Neighborhood: ${neighborhoodName} (Number: ${neighborhoodNumber})
			- Sub-Area: ${subAreaName} (Number: ${subAreaNumber})

	Please adjust these details to create a unique property entry.`
